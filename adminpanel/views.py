...
from .models import GlobalSettings, ContactUS, Navigation, Hiring, Apply
...
def hiring(request):
    glob = GlobalSettings.objects.all()
    hir=Hiring.objects.all()
 
    return render(request, "hiring.html",{'hir':hir, 'glob' : glob})

@login_required(login_url=settings.LOGIN_URL)
def delete_hiring(request, pk):
    hir = get_object_or_404(Hiring, pk=pk)
...
def delete_apply(request, pk):
    appl = get_object_or_404(Apply, pk=pk)
